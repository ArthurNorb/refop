...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex justify-between items-center mb-8">
        ...
        @auth
            <a href="{{ route('artigos.create') }}" class="bg-refop hover:bg-refopEscuro text-white font-bold py-2 px-4 rounded transition-colors">
                Escrever Artigo
            </a>
        @endauth
    </div>
                    ...
                    @if($article->image_path)
                        <div class="md:w-1/3">
                            <a href="{{ route('artigos.show', $article) }}">
                                <img src="{{ Storage::url($article->image_path) }}" alt="{{ $article->title }}" class="h-56 w-full object-cover">
                            </a>
                        </div>
                    @endif
                    <div class="p-6 flex flex-col justify-between flex-1">
                        <div>
                            <h2 class="text-2xl font-bold text-refopEscuro mb-2">
                                <a href="{{ route('artigos.show', $article) }}" class="hover:text-refop">{{ $article->title }}</a>
                            </h2>
                        ...
                        <div class="mt-4">
                            <a href="{{ route('artigos.show', $article) }}" class="text-refop font-semibold hover:underline">Ler mais &rarr;</a>
                        </div>
                    </div>
                </div>
            </div>
